...
console.warn('🔩 bom-functions.js v=cfgid7');
...
function ceLoadPills(pid) {
  ...
  Promise.all([
    ...
    // Populate param ID↔name maps for formula translation
    window._populateParamMap(configs);
    ...
    if (configs.length) {
      ...
      configs.forEach(c => {
        ...
        ctx['cfg_' + c.id] = defVal;  // ID-based alias for param.cfg_{id}
        ...
        pHtml += `<span class="pbs-ce-param-pill param" onclick="ceInsertText('param.${escName}')" title="${escHtml(ptTitle)}">${paramName}${defVal ? '<span class="text-gray-400 ml-0.5">=' + escHtml(String(defVal)) + '</span>' : ''}</span>`;
      });
    ...
    if (varEl) {
      if (vars.length) {
        var vHtml = '';
        vars.forEach(v => {
          // Skip the variable currently being edited (prevents self-reference)
          if (_ceMode === 'variable' && _ceVarEditId && String(v.id) === String(_ceVarEditId)) return;
    ...
    return Promise.all(uniqueIds.map(function(subId) {
      ...
      window.__ceParamContext = ctx;
      // Convert displayed formula from cfg_id to readable names
      var inputEl = document.getElementById('pbs-ce-input');
      if (inputEl && inputEl.value) {
        inputEl.value = window._formulaForDisplay(inputEl.value);
        // Also update CodeMirror if already attached (avoids race with setTimeout)
        if (inputEl.dataset.cmInit && window.CmFormulaEditor) {
          var inst = window.CmFormulaEditor.getCmInstance(inputEl);
          if (inst) { inst.editor.setValue(inputEl.value); }
        }
      }
      ceSchedulePreview();
    });
...
function openCellEditor(itemPk, field, currentVal, isQty, mappingId, expectedType) {
  ...
  if (field === 'variant_name' || field === 'variant_ipn') {
    ...
    title.innerHTML = (labels[field] || '🧬 编辑动态模板') + ' <span style="font-size:11px;background:#fde68a;color:#78350f;padding:2px 7px;border-radius:4px;margin-left:6px;font-family:monospace;font-weight:700;border:2px solid #d97706;white-space:nowrap">v=cfgid7</span>';
    ...
    title.innerHTML = (iconMap[field] || '✏️') + ' ' + (labels[field] || '编辑') + ' <span style="font-size:11px;background:#fde68a;color:#78350f;padding:2px 7px;border-radius:4px;margin-left:6px;font-family:monospace;font-weight:700;border:2px solid #d97706;white-space:nowrap">v=cfgid7</span>';
    input.value = currentVal || '';
...
function openFormulaEditorForVariable(mode, data) {
  ...
  // Title and button
  if (mode === 'add') {
    title.innerHTML = '📐 新建中间变量 <span style="font-size:11px;background:#fde68a;color:#78350f;padding:2px 7px;border-radius:4px;margin-left:6px;font-family:monospace;font-weight:700;border:2px solid #d97706;white-space:nowrap">v=cfgid7</span>';
    document.getElementById('pbs-ce-save-btn').innerHTML = '创建变量';
  } else {
    title.innerHTML = '✏️ 编辑变量 <span style="font-size:11px;background:#fde68a;color:#78350f;padding:2px 7px;border-radius:4px;margin-left:6px;font-family:monospace;font-weight:700;border:2px solid #d97706;white-space:nowrap">v=cfgid7</span>';
...
async function saveCellFormula() {
  ...
  if (_ceMode === 'variable') {
    ...
    const rawFormula = (document.getElementById('pbs-ce-input').value || '').trim();
    const formula = (window._formulaForStorage ? window._formulaForStorage(rawFormula) : rawFormula);
    const description = (document.getElementById('pbs-ce-var-desc').value || '').trim();
    if (!name) { showToast('error', '请输入变量名'); return; }
    // Validate variable name follows identifier rules
    var nameErr = window._validateIdentifier ? window._validateIdentifier(name) : null;
    if (nameErr) { showToast('error', nameErr); return; }
  ...
  // Convert readable names back to cfg_id for storage
  var storageFormula = window._formulaForStorage ? window._formulaForStorage(formula) : formula;

  // Pre-validate formula before saving (non-variant fields only)
  if (st.field !== 'variant_name' && st.field !== 'variant_ipn') {
    statusEl.innerHTML = '<span class="text-gray-400">⏳ 校验公式...</span>';
    if (!(await _preValidateFormula(storageFormula))) return;
  }
  ...
  if (st.field === 'variant_name' || st.field === 'variant_ipn') {
    ...
    var patchData = {};
    patchData[fieldKey] = storageFormula;
...
async function openAddBomItemModal() {
  console.log('[AB] openAddBomItemModal triggered');
  try {
    const pid = configuratorPartId;

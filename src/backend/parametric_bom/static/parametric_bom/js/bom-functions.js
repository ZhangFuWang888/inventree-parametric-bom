...
// ===== DIRECT TEST MODAL =====
// Creates a modal from scratch, bypassing all existing DOM/CSS
function testModalDirect() {
  console.log('[testModalDirect] START');
  
  // Remove existing test overlay if any
  const existing = document.getElementById('direct-test-overlay');
  if (existing) existing.remove();
  
  // Create overlay
  const overlay = document.createElement('div');
  overlay.id = 'direct-test-overlay';
  overlay.style.cssText = 'position:fixed!important;top:0!important;left:0!important;width:100%!important;height:100%!important;background:rgba(0,0,0,0.5)!important;z-index:99999!important;display:flex!important;align-items:center!important;justify-content:center!important;';
  
  // Create box
  const box = document.createElement('div');
  box.style.cssText = 'background:#fff!important;border-radius:12px!important;padding:24px!important;max-width:400px!important;width:90%!important;box-shadow:0 20px 60px rgba(0,0,0,0.3)!important;';
  box.innerHTML = '<h3 style="margin:0 0 16px 0;font-size:16px;font-weight:600;color:#333">🧪 直接测试弹窗</h3><p style="margin:0 0 16px 0;font-size:14px;color:#666">这是用JS直接创建的弹窗！</p><button onclick="document.getElementById(\'direct-test-overlay\').remove()" style="background:#e5e7eb;border:none;border-radius:6px;padding:8px 20px;font-size:13px;cursor:pointer;color:#333">关闭</button>';
  
  overlay.appendChild(box);
  document.body.appendChild(overlay);
  
  console.log('[testModalDirect] DONE - overlay appended');
  return false;
}

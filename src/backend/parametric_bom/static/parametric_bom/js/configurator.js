...
console.warn('⚙️ configurator.js v=cfgid8');
  ...
  Promise.all([
    ...
    if (configs.length) {
      ...
      configs.forEach(c => {
        ...
        html += `<span class="fe-param-pill" onclick="avInsertText('param.${escName}')" title="${escHtml(avPtParts.join('\\\\n'))}">${display}</span>`;
      });
      html += '</div></div>';
    }
...
async function confirmAddVariable() {
  ...
  const rawFormula = document.getElementById('av-formula').value.trim();
...
async function loadPdVariables() {
  ...
  // Load param configs to populate ID↔name mapping for formula display
  try {
    const cfgRes = await apiCall('GET', `part-config/?part=${pid}`);
    const configs = cfgRes.error ? [] : (Array.isArray(cfgRes.data) ? cfgRes.data : (cfgRes.data.results || []));
    window._populateParamMap(configs);
  } catch(e) { console.warn('Failed to load param map for variable list:', e); }
  ...
  let html = `<table class="pd-bom-spreadsheet">
    <thead>
      <tr>
        <th style="width:6%"><span class="col-icon">#</span>ID</th>
        <th style="width:18%"><span class="col-icon">📛</span>变量名</th>
        <th style="width:26%"><span class="col-icon">📐</span>公式</th>
        <th style="width:14%"><span class="col-icon">🔢</span>默认值</th>
        <th style="width:22%"><span class="col-icon">📝</span>描述</th>
        <th style="width:14%"><span class="col-icon">⚙️</span>操作</th>
      </tr>
    </thead>
    <tbody id="pv-tbody">`;
  vars.forEach((v, i) => {
    ...
    const displayFormula = window._formulaForDisplay ? window._formulaForDisplay(v.formula || '') : (v.formula || '');
    const escFmla = escHtml(displayFormula);
    const fmlaShort = displayFormula.length > 30 ? escHtml(displayFormula.substring(0, 30) + '…') : escFmla;
    html += `<tr id="pv-row-${v.id}">
      <td><span class="text-[10px] text-gray-400 font-mono">${v.id}</span></td>

...
console.warn('📝 formula-pages.js v=cfgid7');
...
async function openFormulaEditor(bomItemId, partId, partName) {
  ...
  // Version badge text (badge is in HTML template)
  var feVerEl = document.getElementById('fe-version-badge');
  if (feVerEl) feVerEl.textContent = 'v=cfgid7';
  ...
  // Load available params first (populates _paramCfgToName for formula translation)
  await loadFormulaEditorParams(partId);
  // Then load existing config — values will be converted to readable names
  await loadExistingBomItemConfig(bomItemId);
...
async function loadExistingBomItemConfig(bomItemId) {
  ...
  if (items.length > 0) {
    ...
    if (cfg.qty_formula) document.getElementById('fe-qty-formula').value = (window._formulaForDisplay ? window._formulaForDisplay(cfg.qty_formula) : cfg.qty_formula);
    if (cfg.condition_formula) document.getElementById('fe-condition-formula').value = (window._formulaForDisplay ? window._formulaForDisplay(cfg.condition_formula) : cfg.condition_formula);
    if (cfg.price_formula) document.getElementById('fe-price-formula').value = (window._formulaForDisplay ? window._formulaForDisplay(cfg.price_formula) : cfg.price_formula);
  }
}
...
async function loadFormulaEditorParams(partId) {
  ...
  configs.forEach(c => {
    ...
    feState.paramContext['cfg_' + c.id] = c.default_value;  // ID-based alias
  });
  ...
  // Convert already-loaded formulas from cfg_id to readable names
  ['fe-qty-formula', 'fe-condition-formula', 'fe-price-formula'].forEach(function(id) {
    var el = document.getElementById(id);
    if (el && el.value) {
      el.value = window._formulaForDisplay ? window._formulaForDisplay(el.value) : el.value;
    }
  });
}
...
function insertParam(cfgId, paramName) {
  ...
  if (!ta) return;
  // Insert readable param name — will be converted back to cfg_id on save
...
async function saveBomFormula() {
  ...
  const body = {
    ...
    mode: mode,
    qty_formula: (window._formulaForStorage ? window._formulaForStorage(qtyFormula) : qtyFormula) || '',
    condition_formula: (window._formulaForStorage ? window._formulaForStorage(conditionFormula) : conditionFormula) || '',
    price_formula: (window._formulaForStorage ? window._formulaForStorage(priceFormula) : priceFormula) || '',
  };
